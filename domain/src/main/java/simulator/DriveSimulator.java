...
public class DriveSimulator {
    ...
    public DriveResult simulateDrivesForMatchup(
            GamePlan offensePlan,
            InGamePlayer attacker,
            InGamePlayer defender,
            double assistedDrivePercentage
    ) {
        double advantageRaw = computeAdvantageDrive(attacker.getPlayer(), defender.getPlayer());
    ...
    public DriveResult getDriveTotalContribution(GamePlan offenseTeamGamePlan, GamePlan defenseTeamGamePlan, double assistedShotPercentage) {
        return offenseTeamGamePlan.getPositions().entrySet().stream()
                .filter(entry -> entry.getValue() != null && defenseTeamGamePlan.getPositions().get(entry.getKey()) != null)
                .map(entry -> this.simulateDrivesForMatchup(offenseTeamGamePlan, entry.getValue(), defenseTeamGamePlan.getPositions().get(entry.getKey()), assistedShotPercentage))
