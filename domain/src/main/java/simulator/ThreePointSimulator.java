...
public class ThreePointSimulator {
    ...
    public static ThreePointShootingResult get3ptsFromPlayer(GamePlan homeGamePlan, InGamePlayer inGameOff, InGamePlayer inGameDef, double assistedShotPercentage) {
    ...
    public static ThreePointShootingResult getTotal3ptsContribution(GamePlan home, GamePlan visitor, double playmakingContribution) {
        return home.getPositions().entrySet().stream()
                .filter(entry -> entry.getValue() != null && visitor.getPositions().get(entry.getKey()) != null)
                .map(entry -> ThreePointSimulator.get3ptsFromPlayer(home, entry.getValue(), visitor.getPositions().get(entry.getKey()), playmakingContribution))
