...
public class TwoPointSimulator {
    ...
    public TwoPointShootingResult simulate2ptForMatchup(
        ...
        double advantageRaw = computeAdvantage2pt(shooter.getPlayer(), defender.getPlayer());
    ...
    public TwoPointShootingResult get2ptsTotalContribution(GamePlan offenseTeamGamePlan, GamePlan defenseTeamGamePlan, double assistedShotPercentage) {
        return offenseTeamGamePlan.getPositions().entrySet().stream()
                ...
                .map(entry -> this.simulate2ptForMatchup(offenseTeamGamePlan, entry.getValue(), defenseTeamGamePlan.getPositions().get(entry.getKey()), assistedShotPercentage))
